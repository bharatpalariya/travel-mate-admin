/*
  # Pure Role-Based Admin User Filtering

  1. Changes
    - Remove ALL hardcoded email filtering from get_admin_users()
    - Show ONLY users with role: "admin" in their metadata
    - Keep email fallback ONLY for authentication (is_admin function)
    - Clean separation between authentication and admin list display

  2. Security
    - Maintain RLS policies
    - Keep admin authentication working
    - Pure metadata-based admin user listing
*/

-- Drop existing functions to recreate them with pure role filtering
DROP FUNCTION IF EXISTS get_admin_users();
DROP FUNCTION IF EXISTS debug_user_roles();
DROP FUNCTION IF EXISTS is_admin();

-- Create is_admin function (keeps email fallback for authentication only)
...
BEGIN
  ...
  -- Authentication fallback: specific admin emails (for login purposes only)
...
-- Create function to get ONLY users with admin role in metadata (NO EMAIL FILTERING)
...
BEGIN
  ...
  -- NO HARDCODED EMAIL FILTERING HERE
  RETURN QUERY
  SELECT 
    au.id,
    au.email::text,
...
-- Create debug function to help troubleshoot
...
BEGIN
  ...
  SELECT 
    ...
    CASE 
      ...
      WHEN au.email IN ('admin@travelmate.com', 'amitjaju@gmail.com', 'bharat@travelmate.com') THEN 'admin_email_fallback'
      ELSE 'none'
    END as meets_admin_criteria,
    -- This shows who will appear in the admin list (pure role-based)
    (
      (au.raw_user_meta_data ? 'role' AND au.raw_user_meta_data->>'role' = 'admin')
      OR (au.raw_app_meta_data ? 'role' AND au.raw_app_meta_data->>'role' = 'admin')
    ) as shown_in_admin_list
  FROM auth.users au
  ORDER BY au.created_at DESC;
END;
$$;
